...
import { FontFamilySelector, ThemeToggle, LanguageToggle } from "@/components/ThemeToggle";
import { 
  Palette, 
  Type, 
  Moon, 
...
export default function ThemeShowcase() {
  ...
  const sampleComponents = [
    { type: 'button', variants: ['default', 'secondary', 'outline', 'destructive'] },
    { type: 'badge', variants: ['default', 'secondary', 'outline', 'destructive'] },
  ...
  const sampleText = {
    ar: {
      ...
      paragraph: 'هذا نص تجريبي لعرض الخط العربي المختار. يمكنك تغيير نوع الخط من القائمة أعلاه لمشاهدة الفرق في المظهر والوضوح.',
      short: 'نص قصير',
      numbers: '١٢٣٤٥٦٧٨٩٠'
    },
    en: {
      title: 'Sample English Text',
      paragraph: 'This is a sample text to showcase the selected Arabic font. You can change the font type from the menu above to see the difference in appearance and clarity.',
      short: 'Short text',
      numbers: '1234567890'
    }
  };
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className={`text-center mb-12 ${isRTL ? 'text-right' : 'text-center'}`}>
            ...
            <p className="text-lg text-muted-foreground arabic max-w-3xl mx-auto">
              {isRTL ? 'استكشف جميع إمكانيات التخصيص المتاحة في البيت السوداني' : 'Explore all customization options available in Sudan House'}
            </p>
          </div>
          ...
          <Card className="mb-8 card-dark">
            <CardHeader>
              <CardTitle className={`flex items-center gap-2 arabic ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className={`space-y-3 ${isRTL ? 'text-right' : 'text-left'}`}>
                  ...
                  <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                    ...
                    <span className="text-sm text-muted-foreground arabic">
                      {theme === 'light' ? (isRTL ? 'الوضع النهاري' : 'Light Mode') : (isRTL ? 'الوضع الليلي' : 'Dark Mode')}
                    </span>
                  </div>
                </div>
                ...
                <div className={`space-y-3 ${isRTL ? 'text-right' : 'text-left'}`}>
                  ...
                  <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                    <LanguageToggle />
                    <span className="text-sm text-muted-foreground arabic">
                      {language === 'ar' ? (isRTL ? 'العربية' : 'Arabic') : (isRTL ? 'الإنجليزية' : 'English')}
                    </span>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {colorPalettes.map((palette) => (
                  <div key={palette.name} className="space-y-3">
                    <h3 className={`font-semibold text-foreground arabic ${isRTL ? 'text-right' : 'text-left'}`}>
                      {palette.name} Colors
                    </h3>
              ...
              <div className="space-y-8">
                ...
                <div className={`space-y-4 ${isRTL ? 'text-right' : 'text-left'}`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-foreground arabic">{sampleText[language].short}</p>
                    </div>
                    <div>
                      <Label className="text-sm font-medium arabic">
                        {isRTL ? 'الأرقام' : 'Numbers'}
                      </Label>
                      <p className="text-foreground arabic">{sampleText[language].numbers}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className={`space-y-4 ${isRTL ? 'text-right' : 'text-left'}`}>
                  ...
                  <div className="space-y-2">
                    <p className="text-xs text-muted-foreground arabic">Extra Small (12px)</p>
                    <p className="text-sm text-muted-foreground arabic">Small (14px)</p>
                    <p className="text-base text-foreground arabic">Base (16px)</p>
                    <p className="text-lg text-foreground arabic">Large (18px)</p>
                    <p className="text-xl text-foreground arabic">Extra Large (20px)</p>
                    <p className="text-2xl text-foreground arabic">2XL (24px)</p>
                    <p className="text-3xl text-foreground arabic">3XL (30px)</p>
                  </div>
                </div>
              </div>
            </CardContent>
                ...
                <div className={`space-y-4 ${isRTL ? 'text-right' : 'text-left'}`}>
                  ...
                  <div className={`flex flex-wrap gap-4 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm arabic">{isRTL ? 'مكتمل' : 'Complete'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Star className="w-5 h-5 text-yellow-500" />
                      <span className="text-sm arabic">{isRTL ? 'مفضل' : 'Favorite'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Heart className="w-5 h-5 text-red-500" />
                      <span className="text-sm arabic">{isRTL ? 'أعجبني' : 'Like'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <MessageCircle className="w-5 h-5 text-blue-600" />
                      <span className="text-sm arabic">{isRTL ? 'رسالة' : 'Message'}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className={`space-y-3 ${isRTL ? 'text-right' : 'text-left'}`}>
                  ...
                  <p className="text-sm text-muted-foreground arabic">
                    {isRTL ? 'التصميم محسّن للهواتف الذكية مع نصوص واضحة وأزرار قابلة للنقر بسهولة' : 'Design optimized for smartphones with clear text and easily tappable buttons'}
                  </p>
                </div>
                <div className={`space-y-3 ${isRTL ? 'text-right' : 'text-left'}`}>
                  <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                  ...
                  <p className="text-sm text-muted-foreground arabic">
                    {isRTL ? 'استغلال أمثل للمساحة المتاحة مع تجربة مستخدم محسّنة للشاشات الكبيرة' : 'Optimal use of available space with enhanced user experience for large screens'}
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          <Card className="card-dark">
            <CardHeader>
              <CardTitle className={`flex items-center gap-2 arabic ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                🇸🇩
                {isRTL ? 'العناصر الثقافية السودانية' : 'Sudanese Cultural Elements'}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-6">
                <div className={`grid grid-cols-1 md:grid-cols-3 gap-4 ${isRTL ? 'text-right' : 'text-left'}`}>
                  <div className="p-4 bg-gradient-to-br from-primary-500 to-primary-600 rounded-lg text-white">
                    ...
                    <p className="text-sm text-white/80 arabic">
                      {isRTL ? 'مستوحى من نهر النيل العظيم' : 'Inspired by the great Nile River'}
                    </p>
                  </div>
                  <div className="p-4 bg-gradient-to-br from-secondary-500 to-secondary-600 rounded-lg text-white">
                    ...
                    <p className="text-sm text-white/80 arabic">
                      {isRTL ? 'يعكس جمال الصحراء السودانية' : 'Reflects the beauty of Sudanese desert'}
                    </p>
                  </div>
                ...
                <div className={`text-center py-8 bg-sudanese-pattern rounded-lg ${isRTL ? 'text-right' : 'text-center'}`}>
                  ...
                  <p className="text-muted-foreground arabic">
                    {isRTL ? 'حيث التراث يلتقي بالحداثة' : 'Where heritage meets modernity'}
                  </p>
                </div>
              </div>
            </CardContent>
