      ...
      <section className="relative">
        ...
        <div className="relative container mx-auto px-4 py-12 md:py-20 lg:py-28">
          <div className={`max-w-4xl mx-auto text-center ${isRTL ? 'text-right' : 'text-center'}`}>
            ...
            {/* CTA Buttons */}
            <div className={`flex flex-col xs:flex-row gap-3 md:gap-4 justify-center mb-8 md:mb-12 px-4 ${isRTL ? 'xs:flex-row-reverse' : ''}`}>
              <Link to="/marketplace" className="w-full xs:w-auto">
                <Button size="lg" className="button-cultural w-full xs:w-auto px-6 md:px-8 py-3 md:py-4 text-base md:text-lg arabic">
                  استكشف السوق
                  <ArrowIcon className={`w-4 h-4 md:w-5 md:h-5 ${isRTL ? 'mr-2' : 'ml-2'}`} />
                </Button>
              </Link>
              <Link to="/register" className="w-full xs:w-auto">
                <Button variant="outline" size="lg" className="w-full xs:w-auto px-6 md:px-8 py-3 md:py-4 text-base md:text-lg bg-white/10 backdrop-blur-sm border-white/20 text-white hover:bg-white hover:text-primary-700 arabic">
                  انضم إلينا
                </Button>
              </Link>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 lg:gap-8">
              {heroStats.map((stat, index) => (
                <div key={index} className="text-center animate-slide-up" style={{ animationDelay: `${index * 0.1}s` }}>
                  ...
                  <div className="text-xs md:text-sm lg:text-base text-white/80 arabic px-1">{stat.label}</div>
                </div>
              ))}
            </div>
          </div>
            ...
            {services.map((service, index) => (
              <Card key={index} className="card-dark hover:shadow-xl transition-all duration-300 hover:scale-105 group animate-fade-in" style={{ animationDelay: `${index * 0.1}s` }}>
                <CardHeader className="text-center pb-4">
                  <div className={`w-12 h-12 md:w-16 md:h-16 mx-auto ${service.bgColor} rounded-xl flex items-center justify-center mb-3 md:mb-4 group-hover:scale-110 transition-transform duration-300`}>
                    <service.icon className={`w-6 h-6 md:w-8 md:h-8 bg-gradient-to-br ${service.color} bg-clip-text text-transparent`} />
                  </div>
                  <CardTitle className={`text-lg md:text-xl font-bold arabic ${isRTL ? 'text-right' : 'text-center'}`}>
                    {service.title}
                  </CardTitle>
                </CardHeader>
                <CardContent className={`${isRTL ? 'text-right' : 'text-center'} pb-6`}>
                  <p className="text-sm md:text-base text-muted-foreground arabic mb-4 md:mb-6 leading-relaxed">
                    {service.description}
                  </p>
                  <Link to={service.href}>
                    <Button className={`w-full bg-gradient-to-r ${service.color} hover:opacity-90 transition-opacity arabic text-sm md:text-base`}>
                      اكتشف المزيد
            ...
            {testimonials.map((testimonial, index) => (
              <Card key={index} className="card-dark animate-fade-in" style={{ animationDelay: `${index * 0.2}s` }}>
                <CardContent className="p-4 md:p-6">
                  <div className={`flex items-center gap-1 mb-3 md:mb-4 ${isRTL ? 'flex-row-reverse justify-end' : 'justify-start'}`}>
                    {[...Array(testimonial.rating)].map((_, i) => (
                      <Star key={i} className="w-4 h-4 md:w-5 md:h-5 fill-yellow-400 text-yellow-400" />
                    ))}
                  </div>
                  <p className={`text-sm md:text-base text-muted-foreground arabic mb-4 md:mb-6 leading-relaxed ${isRTL ? 'text-right' : 'text-left'}`}>
                    "{testimonial.content}"
                  </p>
                  <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                    ...
                    <div className={isRTL ? 'text-right' : 'text-left'}>
                      <div className="font-semibold text-foreground arabic text-sm md:text-base">{testimonial.name}</div>
                      <div className="text-xs md:text-sm text-muted-foreground arabic">{testimonial.role}</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
