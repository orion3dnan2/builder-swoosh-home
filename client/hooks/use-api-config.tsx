...
export const useApiConfig = () => {
  ...
  const [activeConfig, setActiveConfig] = useState<ApiConfiguration | null>(
    null,
  );
  const [stats, setStats] = useState<ApiConfigurationStats>({
    total: 0,
    active: 0,
    inactive: 0,
    healthy: 0,
  ...
  const updateConfig = useCallback(
    async (id: string, updates: Partial<ApiConfiguration>) => {
      try {
        setLoading(true);
  ...
  const setActiveConfiguration = useCallback(
