...
export function ApiConnectionStatus({ 
  ...
  const { isExternal, activeConfig, baseUrl, lastCheck, refreshStatus } = useApiConnectionStatus();
  ...
  const getStatusIcon = () => {
    ...
    if (connectionTest.success) {
      return isExternal ? <Globe className="w-4 h-4" /> : <Server className="w-4 h-4" />;
    }
  ...
  const getDetailedInfo = () => {
    return {
      ...
      responseTime: connectionTest?.responseTime ? `${connectionTest.responseTime}ms` : '-',
        ...
        <Tooltip>
          ...
          <TooltipContent side="bottom" className="arabic">
            <div className="space-y-1">
              <p><strong>الخادم:</strong> {getDetailedInfo().serverName}</p>
              <p><strong>النوع:</strong> {getDetailedInfo().serverType}</p>
              <p><strong>الحالة:</strong> {getDetailedInfo().status}</p>
              {connectionTest?.responseTime && (
                <p><strong>وقت الاستجابة:</strong> {getDetailedInfo().responseTime}</p>
              )}
              <p><strong>آخر فحص:</strong> {getDetailedInfo().lastCheck}</p>
            </div>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
        ...
        {connectionTest && !connectionTest.success && (
          <div className="mt-3 p-2 bg-red-50 border border-red-200 rounded-lg">
            <div className="flex items-center space-x-2 space-x-reverse">
              ...
              <span className="text-sm text-red-700 arabic">خطأ في الاتصال:</span>
            </div>
            <p className="text-sm text-red-600 mt-1 arabic">{connectionTest.message}</p>
          </div>
        )}
        ...
        {connectionTest && connectionTest.success && (
          <div className="mt-3 p-2 bg-green-50 border border-green-200 rounded-lg">
            <div className="flex items-center space-x-2 space-x-reverse">
              ...
              <span className="text-sm text-green-700 arabic">الاتصال ناجح</span>
              {connectionTest.responseTime && (
                <span className="text-xs text-green-600">({connectionTest.responseTime}ms)</span>
              )}
            </div>
          </div>
        )}
...
export function QuickApiStatus() {
  return <ApiConnectionStatus variant="badge" showRefresh={false} autoRefresh={true} />;
}

// Detailed status component for settings/admin pages
export function DetailedApiStatus() {
  return <ApiConnectionStatus variant="full" showRefresh={true} autoRefresh={true} />;
}
